...
type TaskProps = {
  ...
  onUpdate: (taskId: string, title: string, description: string, dueDate: string) => Promise<void>;
};
...
export default function Task({ task, onDelete, onUpdate }: TaskProps) {
  ...
  const [dueDate, setDueDate] = useState(task.due_date || ""); // New state for due date
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if ((title.trim() && title !== task.title) || (description.trim() !== task.description) || (dueDate !== task.due_date)) {
      await onUpdate(task.id, title.trim(), description.trim(), dueDate);
    }
    setIsEditing(false);
  };
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div ref={modalRef} className="bg-[#161b22] rounded-lg shadow-2xl border border-[#30363d] w-full max-w-md mx-4 overflow-hidden relative">
            ...
            <div className="flex justify-between items-center border-b border-[#30363d] p-4">
              ...
              <button onClick={closeModal} className="text-gray-400 hover:text-gray-200 text-xl">
                ×
              </button>
            </div>
              ...
              {isEditing ? (
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="flex justify-end gap-2">
                    <button type="button" onClick={closeModal} className="px-3 py-1.5 text-sm bg-[#21262d] text-gray-300 rounded-md hover:bg-[#30363d] transition-colors">
                      Cancel
                    </button>
                    <button type="submit" className="px-3 py-1.5 text-sm bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-md hover:from-blue-700 hover:to-blue-800 transition-colors">
                      Save
                    </button>
                  </div>
                </form>
